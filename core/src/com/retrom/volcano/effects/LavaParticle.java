...
public class LavaParticle extends Particle {
	...
	public LavaParticle(Array<Sprite> sprites, Vector2 position) {
		super(sprites.random(), getDuration(), position.cpy(), getInitVel());
	...
	protected void childSpecificUpdating(float deltaTime) {
		...
		if (this.position_.y < original_y) {
			this.destroy();
		}
	}
	...
	public float getAlpha() {
		return Math.min(1, Math.max(0, (this.position_.y - original_y) / 35));
